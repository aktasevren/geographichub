...
type Props = {
  ...
  align?: "left" | "right";
  locale?: "tr" | "en";
  /** compact mode: flags + side label + result only; no country list or commanders */
  compact?: boolean;
  /** cap the flags rendered (e.g. compact mode) */
  maxFlags?: number;
  className?: string;
};
...
function CoatFallback({ monogram }: { monogram: string }) {
  return (
    <svg
      ...
      style={{ aspectRatio: "3 / 2", background: "var(--war-ink-2)" }}
    >
      <path
...
function FlagWithFallback({ src, alt, monogram }: { src: string; alt: string; monogram: string }) {
  const [failed, setFailed] = React.useState(false);
  if (failed) return <CoatFallback monogram={monogram} />;
  return (
...
function resultLabel(r: WarSide["result"], locale: "tr" | "en"): string {
...
function FlagStrip({
  ...
  const flagsToShow = maxFlags ? side.countries.slice(0, maxFlags) : side.countries;
  const hiddenCount = side.countries.length - flagsToShow.length;

  return (
    <div
      className={wn(
        "war-flag-group flex flex-col",
    ...
    >
      ...
      >
        {flagsToShow.map((c, i) => {
          const mono = monogramOf(c.name);
          if (c.flagFallback === "coat" || !c.flagUrl) {
            return <CoatFallback key={`${c.name}-${i}`} monogram={mono} />;
          }
          return (
            <FlagWithFallback
              key={`${c.name}-${i}`}
      ...
      >
        ...
        <span
          style={{
            ...
            color:
              side.result === "victor" ? "var(--war-gold)" : "var(--war-paper-3)",
